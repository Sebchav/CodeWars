/*

Create a function that takes a Roman numeral as its argument and returns
its value as a numeric decimal integer. You don't need to validate the form
of the Roman numeral. Modern Roman numerals are written by expressing each 
decimal digit of the number to be encoded separately, starting with the leftmost 
digit and skipping any 0s. So 1990 is rendered "MCMXC" (1000 = M, 900 = CM, 90 = XC)
and 2008 is rendered "MMVIII" (2000 = MM, 8 = VIII). The Roman numeral for 1666, "MDCLXVI"
, uses each letter in descending order.

Example:

solution('XXI'); // should return 21
Help:

Symbol    Value
I          1
V          5
X          10
L          50
C          100
D          500
M          1,000

*/

function convertirCaracterAEntero( caracter ){
    switch(caracter){
        case "I": return 1;
        case "V": return 5;
        case "X": return 10;
        case "L": return 50;
        case "C": return 100;
        case "D": return 500;
        case "M": return 1000;
        default: return 0;
    }

}

function romanoAEntero(romano) {
    let numero = convertirCaracterAEntero(romano.charAt(0));
    let valorAnterior;
    let valorActual;

    for(let i = 1; i < romano.length; i++){
        valorActual = convertirCaracterAEntero(romano.charAt(i));
        valorAnterior = convertirCaracterAEntero(romano.charAt(i - 1));

        if(valorActual <= valorAnterior){
            numero += valorActual;
        } else{
            numero = numero - valorAnterior * 2 + valorActual;
        }
    }

    return numero;

}

console.log(romanoAEntero("XXI"));

